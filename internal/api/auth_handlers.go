...
// the main qui-sync UI for visual consistency.
...
<head>
<meta charset="utf-8">
<title>qui-sync — First-run setup</title>
...
<body>
<div class="card">
  <h1>Welcome to qui-sync</h1>
  <p class="sub">Authentication is now required. Create the admin account to continue — you'll use this login every time you access qui-sync from outside your local network.</p>
  ...
  <footer>qui-sync {{.Version}} · <a href="https://github.com/prophetse7en/qui-sync" style="color:#4a90e2;text-decoration:none">github</a></footer>
</div>
</body>
</html>`))
...
<div class="card">
  ...
  <footer>qui-sync {{.Version}}</footer>
</div>
</body>
</html>`))
...
func (h *AuthHandlers) handleAuthStatus(w http.ResponseWriter, r *http.Request) {
	...
	// fails later.
	//
	// CONTAINER-LOCAL note: the ModeNone branch leaks username + lock-
	// state on /api/auth/status to anyone in the network, by design.
	// AUTH_MODE=none is gated behind I_UNDERSTAND_AUTH_IS_DISABLED=yes
	// at startup (auth.ValidateConfig) so an admin opting in already
	// accepts the implication "no auth means no auth".
...
// InitAuth wires the auth subsystem against qui-sync's live config.
// getCfg is a snapshot-returning accessor — qui-sync uses the flat
// *core.Config served by api.Server.GetConfig() rather than clonarr's
// ConfigStore wrapper, so we adapt at this single boundary instead of
// forcing the wrapper across the rest of the app. version is shown in
// the setup/login footer; mux is the router this handler family
// registers onto.
func InitAuth(ctx context.Context, getCfg func() *core.Config, version string, mux *http.ServeMux) *auth.Store {
	...
	// /api/health intentionally NOT registered here — qui-sync's
	// Server.RegisterRoutes owns the canonical handler (with version +
	// uptime fields), and the auth-package allowlist treats the path as
	// public so Docker healthchecks bypass auth regardless.
	return store
}
